...
def main():
    ...
    for i in range(5):
        ...
        if train:
            ...
            torch.save(mflow.state_dict(), f"../data/models/mflow_cifar10_normalized_dim{d}_run{i}.pt")
            print(f"Model saved!")
        else:
            mflow.load_state_dict(torch.load(f"../data/models/mflow_cifar10_normalized_dim{d}_run{i}.pt"))
        ...
        torch.save(losses_eval, f"../data/models/mflow_cifar10_normalized_dim{d}_losses_eval_run{i}.pt")
        torch.save(losses_train, f"../data/models/mflow_cifar10_normalized_dim{d}_losses_train_run{i}.pt")

        # Write current d and runtime to a .txt file
        with open(f"dims_runtime_log_nsamples{n_samples}_normalized.txt", "a") as log_file:  # Open the file in append mode
