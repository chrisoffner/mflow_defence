...
bs = 100
subset_indices = range(100) # TODO: remove and load whole dataset
...
results = { atk: torch.zeros(act_num_epsilons, 4) for atk in attacks }
...
for idx_eps, eps in enumerate(epsilons):
    ...
    for x_clean, labels in clean_loader:
        # get adversarial attack sample for attacks
        x_atk = {}
        x_def = {}
        for atk in attacks:
...
for atk in attacks:
    ...
    plt.tight_layout()
    plt.savefig(figures_dir / f"rel_freq_pixeldefend_{atk}.pdf")